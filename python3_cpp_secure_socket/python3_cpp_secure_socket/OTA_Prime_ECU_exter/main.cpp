...
#include <v0/commonapi/OTA_exterProxy.hpp>
...
int main(){
    ...
    std::shared_ptr<OTA_exterProxy<>> myProxy_exter = runtime->buildProxy<OTA_exterProxy>(domain, instance_exter);
    ...
    std::shared_ptr<OTA_interProxy<>> myProxy_inter = runtime->buildProxy<OTA_interProxy>(domain, instance);
    ...
    std::ifstream file("./OTA_Prime_ECU_exter/gui_Test", std::ios::binary);
    ...
    myProxy_inter->pushUpdateAsync(firmware, signature,
        [](const CommonAPI::CallStatus& status, const int32_t& result) {
            if (status == CommonAPI::CallStatus::SUCCESS) {
                std::cout << "[Async] inter pushUpdate 성공, result = " << result << std::endl;
            } else {
